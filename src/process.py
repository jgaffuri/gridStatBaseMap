...
def export_map_to_geotiff(project_path, output_path, extent, scale, dpi=96):
    """
    Exporte la carte d'un projet QGIS au format GeoTIFF.
    ...
    map.setExtent(QgsRectangle(*extent)) # Remplace `extent` par tes coordonnées (xmin, ymin, xmax, ymax)
    map.setScale(scale)
